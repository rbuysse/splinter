...
use crate::network::peer_manager::peer_map::{PeerMap, PeerStatus, INITIAL_RETRY_FREQUENCY};
        ...
        match peer_metadata.status {
            ...
            PeerStatus::Connected => {
                ...
                if local_identity > identity.as_str() {
                    ...
                    info!(
                        "Replacing outbound connection for inbound for peer {}",
                        peer_metadata.id
                    );
                }
            }
...
fn handle_connected(
    ...
    if let Some(mut peer_metadata) = peers.get_peer_from_endpoint(&endpoint).cloned() {
        match peer_metadata.status {
            ...
            PeerStatus::Connected => {
                // Compare identities, if local identity is , close incoming connection
                // otherwise, remove outbound connection and replace with inbound.
                if local_identity < identity.as_str() {
                    info!(
