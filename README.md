...
`docker-compose  --env-file $(pwd)/.env -f docker-compose-installed.yaml build`
from the root directory. This command builds Docker images for all of the
...
`docker-compose --env-file $(pwd)/.env -f docker-compose-installed.yaml build <component>`
from the root directory. For example, to build only the splinter daemon, run
`docker-compose --env-file $(pwd)/.env -f docker-compose-installed.yaml build splinterd`.
